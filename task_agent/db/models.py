...
class User(Base):
    ...
    language_pref: Mapped[str] = mapped_column("language", String(10), default="en", nullable=False)
...
class DynamicTaskLog(Base):
    ...
    task_date: Mapped[date] = mapped_column(Date, nullable=False, default=date.today)
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column("expired_at", DateTime)
    reward_points: Mapped[int] = mapped_column(Integer, default=0)
    __table_args__ = (
        UniqueConstraint("user_id", "task_date", name="uq_dtl_user_date"),
        Index("idx_dtl_user_status", "user_id", "task_status"),
        Index("idx_dtl_expires_status", "expired_at", "task_status"),
    )
...
class RoutineTaskLog(Base):
    ...
    reward_points: Mapped[int] = mapped_column(Integer, default=0)
    __table_args__ = (
        UniqueConstraint("user_id", "task_type", "period", name="uq_rtl_user_type_period"),
        Index("idx_rtl_user_status", "user_id", "task_status"),
    )
