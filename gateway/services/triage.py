...
from alert_db.models import UserCGMLog, UserHRLog, UserWeeklyPattern
...
async def evaluate_soft_triggers(
    ...
    celery_app.send_task(
        "alert_agent.tasks.run_investigation",
...
async def _find_upcoming_activity(
    ...
    # In demo mode, skip day_of_week check so the demo works on any day
    filters = [
    ...
    if not settings.demo_mode:
        ...
        filters.append(UserWeeklyPattern.day_of_week == day_of_week)

    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(UserWeeklyPattern).where(*filters)
        )
