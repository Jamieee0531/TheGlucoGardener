...
async def list_intervention_log(
    user_id: str,
    limit: int = Query(20, ge=1, le=200),
    today_only: bool = Query(False),
) -> list[dict]:
    async with AsyncSessionLocal() as session:
        stmt = select(InterventionLog).where(InterventionLog.user_id == user_id)
        if today_only:
            from datetime import datetime, time as dt_time
            today_start = datetime.combine(datetime.now().date(), dt_time(0, 0))
            stmt = stmt.where(InterventionLog.triggered_at >= today_start)
        result = await session.execute(
            stmt.order_by(InterventionLog.triggered_at.desc()).limit(limit)
        )
        logs = result.scalars().all()
    return [
        {
